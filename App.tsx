...
function App() {
  ...
  const [shareFlowMessage, setShareFlowMessage] = useState<string>(''); // Required for share flow specific messages
  ...
  const resetP2PReceiveStates = (reason?: string, context: 'manual_p2p_receiver' | 'share_flow_receiver' | 'manual' = 'manual_p2p_receiver') => {
    if (p2pReceiveTimeoutIdRef.current) { 
      clearTimeout(p2pReceiveTimeoutIdRef.current); 
      p2pReceiveTimeoutIdRef.current = null; 
    }
    setP2PReceivingFileMeta(null); 
    setP2PReceivedFileChunks([]); 
    setP2PFileTransferProgress(null);
    
    if (reason) {
      const systemMessageText = `System (${context}): ${reason}`;
      const systemMessage = { timestamp: Date.now(), data: systemMessageText, sender: 'system' as const };
      if (context === 'manual_p2p_receiver' || context === 'manual') {
        setManualP2PReceivedMessages(prev => [...prev, systemMessage]);
      }
      setAppMessageFx(reason, 'error', 0);
    }
  };
  ...
  const commonP2POnDataHandler = async (data: any, p2pContext: 'manual' | 'manual_p2p_receiver' | 'share_flow_receiver') => {
            ...
            try {
                await performP2PSendAndUpdateState(service, item as File, 
                    ...
                    () => { // onComplete
                        ...
                        setItemToShare(null); setShareTargetRoomIdInput(''); 
      ...
      <details className="section-details" open={itemToShare !== null || (shareFlowStep !== 'idle' && shareFlowStep !== 'share_complete_success') }>
          ...
          <div className="section-container unified-share-section">
              ...
              <div className="share-target-input-section">
                  ...
                  <button onClick={cancelShareProcess} disabled={['p2p_direct_transfer', 'r2_fallback_upload'].includes(shareFlowStep)}>
